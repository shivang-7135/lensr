    ...
    <div className="space-y-6">
      ...
      {(data.scene || data.mood) && (
        <div className="p-5 glass-strong">
          ...
          {data.mood && (
            <div className="text-xs uppercase tracking-widest text-accent mt-2">
              Mood: {data.mood}
            </div>
          )}
        </div>
      ...
      {!!data.captions?.length && (
        <div className="grid md:grid-cols-2 gap-3">
          {data.captions.map((c, i) => (
            <div
              key={i}
              className="p-4 glass glass-hover space-y-2 fade-up"
        ...
        <div className="p-4 glass">
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.hashtags.map((h, i) => (
              <span key={i} className="text-sm px-2 py-0.5 rounded-full glass-soft">
                {h}
              </span>
            ))}
          </div>
